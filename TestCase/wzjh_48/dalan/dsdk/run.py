...
from TestCase.wzjh_48.dalan.dsdk import denglu,open_game,order_wechat
import time
...
def _run(device,name,pkg_id):
    ...
    open_apk=eval('object_1.wzjh_open_'+device)()
    if open_apk[0]==True:
...
def _denglu(device,package_name,pkg_id):
    ...
    wzjh_denglu=eval('object_2.wzjh_deng_lu_'+device)()
    if wzjh_denglu[0]==True:
        report_API.get_images({'model':devie_xinxi['model'], 'imei':devie_xinxi['imei'], 'system_version':devie_xinxi['system_version'], 'network':devie_xinxi['network']}, pkg_id, '登录', '登录游戏成功',1,node_start_time_5,wzjh_denglu[2])
        return True
    elif wzjh_denglu[0]==False:
        report_API.get_images({'model':devie_xinxi['model'], 'imei':devie_xinxi['imei'], 'system_version':devie_xinxi['system_version'], 'network':devie_xinxi['network']}, pkg_id, '登录', '登录游失败：'+wzjh_denglu[1], 0,node_start_time_5,wzjh_denglu[2])
        return False
    else:
        return False
...
if __name__ == '__main__':
    ...
    # p2.join()
    # print('主进程结束')
    print(_run('b5940085','com.wjfy.wzjh','2020'))
    #print(_denglu('54a4c6159804','com.dalan.zyrg.toutiao2','2020'))
