...
from TestCase.fzzj_61.dalan.dsdk import get_order_log
...
class Order(object):
    ...
    def place_order_b5940085(self):
        '''小米9_调起微信'''
        touch(Template(r"fzzj_游戏内充值.png", record_pos=(0.288, -0.196), resolution=(2340, 1080)))
        if exists(Template(r"fzzj_充值档位6元.png", record_pos=(-0.32, 0.037), resolution=(2340, 1080))):
            print('{}_已在充值档位页面'.format(self.device))
             #创建线程获取日志
            t = threading.Thread(target=get_order_log.say,args=('eve',self.device))
            t.setDaemon(True)
            t.start()
            touch(Template(r"fzzj_充值档位6元.png", record_pos=(-0.32, 0.037), resolution=(2340, 1080)))
            if  self.poco(text="微信").wait(7).exists():
                print('{}--{}拉起支付方式弹窗成功'.format(os.getpid(),self.device))
                self.poco(text="微信").click()
                time.sleep(5)
